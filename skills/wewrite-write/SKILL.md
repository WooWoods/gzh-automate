...
执行 `wewrite-title`。它读取 `artifacts.brief` 和 `artifacts.claims`，按内容类型生成
多风格候选标题、评分并推荐主标题和备选。将推荐的主标题用于初稿。
...
- 标题：先执行 `wewrite-title` 基于任务书和主张清单生成候选标题，选取主标题后再写初稿。
  如果用户已指定标题则跳过标题策略，直接使用。
- 正文通常 1200-2500 字，可按任务书调整。
